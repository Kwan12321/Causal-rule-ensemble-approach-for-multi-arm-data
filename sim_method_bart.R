...
########################## T-learner (random bart)  ################################

tbart <- function(sim.train, sim.test, seed = 1){
  ...
  for(w in K.ind){
    ...
    fit = BART::wbart(y.train = y.train, x.train = as.matrix(x.train))
    y_pred <- apply(predict(fit, as.matrix(X.test)),2,mean)
...
########################## S-learner (random bart)  ################################

sbart <- function(sim.train, sim.test, seed = 1){
  ...
  for(w in K.ind){
    y_pred <- apply(predict(fit, as.matrix(cbind(W = w, X.test))),2,mean)
    res[,which(K.ind == w)] <- y_pred
  }
...
########################## X-learner: naive extension of X-learner (random bart)  ##

xbart <- function(sim.train, sim.test, W.hat, seed = 1){
  ...
  for(w in K.ind){
    ...
    fit = BART::wbart(y.train = y.train, x.train = as.matrix(x.train))
    mu_pred <- apply(predict(fit, as.matrix(X.train[which(W.train!=w),])),2,mean)
  ...
  for(g in 2:(length(K.ind))){
    ...
    t_fit_k = BART::wbart(y.train = dk, x.train = as.matrix(X.train[which(W.train == K.ind[g]),]))
    t_fit_0 = BART::wbart(y.train = d0, x.train = as.matrix(X.train[which(W.train == 0),]))
    
    tau_k_pred <- apply(predict(t_fit_k, as.matrix(X.test)),2,mean)
    tau_0_pred <- apply(predict(t_fit_0, as.matrix(X.test)),2,mean)
...
########################## m-learner (random bart)  ################################

mbart <- function(sim.train, sim.test, W.hat, seed = 1){
  ...
  for(g in 1:(length(K.ind)-1)){
    ...
    fit = BART::wbart(y.train = zk, x.train = as.matrix(X.train))
    tau_pred <- apply(predict(fit, as.matrix(X.test)),2,mean)
    tau.can[,g] <- tau_pred
  }
...
######################### DR-learner (random bart)  ################################

drbart <- function(sim.train, sim.test, W.hat, seed = 1){
  ...
  for(w in K.ind){
    ...
    fit = BART::wbart(y.train = y.train, x.train = as.matrix(x.train))
    y_pred <- apply(predict(fit, as.matrix(X.train)),2,mean)
...
rfbart <- function(sim.train, sim.test, W.hat, W.hat.test, seed = 1){
  ...
  fit = BART::wbart(y.train = Y.train, x.train = as.matrix(X.train))
  m.train <- apply(predict(fit, as.matrix(X.train)),2,mean)
...
rbart <- function(sim.train, sim.test, W.hat, seed = 1){
