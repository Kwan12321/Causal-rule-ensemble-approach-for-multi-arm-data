...
tboost <- function(sim.train, sim.test, seed = 1){
  ...
  for(w in K.ind){
    ...
    dtrain <- xgboost::xgb.DMatrix(data = data.matrix(x.train), label = y.train)
    fit <- xgboost::xgb.train(data = dtrain, nrounds = 100)
    y_pred <- predict(fit, newdata = as.matrix(X.test))
...
sboost <- function(sim.train, sim.test, seed = 1){
  ...
  for(w in K.ind){
    y_pred <- predict(fit, newdata = as.matrix(cbind(W = w, X.test)))
    res[,which(K.ind == w)] <- y_pred
  }
...
########################## X-learner: naive extension of X-learner (xgboost)  ##

xboost <- function(sim.train, sim.test, W.hat, seed = 1){
  ...
  for(w in K.ind){
    ...
    dtrain <- xgboost::xgb.DMatrix(data = data.matrix(x.train), label = y.train)
    fit <- xgboost::xgb.train(data = dtrain, nrounds = 100)
    mu_pred <- predict(fit, newdata = as.matrix(X.train[which(W.train!=w),]))
  ...
  for(g in 2:(length(K.ind))){
    ...
    dtraink <- xgboost::xgb.DMatrix(data = data.matrix(X.train[which(W.train ==  K.ind[g]),]), label = dk)
    t_fit_k <- xgboost::xgb.train(data = dtraink, nrounds = 100)
    
    dtrain0 <- xgboost::xgb.DMatrix(data = data.matrix(X.train[which(W.train == 0),]), label = d0)
    t_fit_0 <- xgboost::xgb.train(data = dtraink, nrounds = 100)
    
    tau_k_pred <- predict(t_fit_k, newdata = as.matrix(X.test))
    tau_0_pred <- predict(t_fit_0, newdata = as.matrix(X.test))
...
mboost <- function(sim.train, sim.test, W.hat, seed = 1){
  ...
  for(g in 1:(length(K.ind)-1)){
    ...
    dtrain <- xgboost::xgb.DMatrix(data = data.matrix(X.train), label = zk)
    fit <- xgboost::xgb.train(data = dtrain, nrounds = 100)
    tau_pred <- predict(fit, newdata = as.matrix(X.test))
    tau.can[,g] <- tau_pred
  }
...
drboost <- function(sim.train, sim.test, W.hat, seed = 1){
  ...
  for(w in K.ind){
    ...
    dtrain <- xgboost::xgb.DMatrix(data = data.matrix(x.train), label = y.train)
    fit <- xgboost::xgb.train(data = dtrain, nrounds = 100)
    y_pred <- predict(fit, newdata = as.matrix(X.train))
    
    zk <- (Y.train - y_pred)*as.numeric(W.train == w)/W.hat[,w+1] + y_pred
    ztrain <- xgboost::xgb.DMatrix(data = data.matrix(X.train), label = zk)
    z_fit <-  xgboost::xgb.train(data = ztrain, nrounds = 100)
    tau_pred <- predict(z_fit, newdata = as.matrix(X.test))
...
rfboost <- function(sim.train, sim.test, W.hat, W.hat.test, seed = 1){
  ...
  dtrain <- xgboost::xgb.DMatrix(data = data.matrix(X.train), label = Y.train)
  fit <- xgboost::xgb.train(data = dtrain, nrounds = 100)
  m.train <- predict(fit, newdata = as.matrix(X.train))
...
rboost <- function(sim.train, sim.test, W.hat, seed = 1){
  ...
  adj.pi = W.ind - pi.hat
  x.tilde = matrix(as.vector(apply(adj.pi, 2, function(j) j * cbind(1,as.matrix(X.train)))), nrow = nrow(X.train), byrow = FALSE)
  penalty_factor = rep(c(0, array(1, pobs)), length(K.grp)-1)
  fit.R = glmnet::cv.glmnet(x.tilde, y.adj, penalty.factor = penalty_factor, family = "gaussian", maxit = 100000, intercept = FALSE)
  gc(verbose = FALSE)
